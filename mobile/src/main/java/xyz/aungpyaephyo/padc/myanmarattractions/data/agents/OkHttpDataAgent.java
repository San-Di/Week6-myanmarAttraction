...
public class OkHttpDataAgent implements AttractionDataAgent {
    ...
    private OkHttpDataAgent() {
        mHttpClient = new OkHttpClient.Builder()    //Builder pattern    //step 1 .
                .connectTimeout(15, TimeUnit.SECONDS)
                .writeTimeout(15, TimeUnit.SECONDS)
                .readTimeout(60, TimeUnit.SECONDS) //shouldn't be over 15 seconds
                .build();
    }
    ...
    public void loadAttractions() {
        new AsyncTask<Void, Void, List<AttractionVO>>() {
            ...
            protected List<AttractionVO> doInBackground(Void... voids) {
                RequestBody formBody = new FormBody.Builder()   //Step 2.//okHttp removes InputStream part
                        .add(MyanmarAttractionsConstants.PARAM_ACCESS_TOKEN, MyanmarAttractionsConstants.ACCESS_TOKEN) //this line count is depend on the number of para u wanna pass
                        .build();

                Request request = new Request.Builder() //step 3.
                        .url(MyanmarAttractionsConstants.ATTRACTION_BASE_URL + MyanmarAttractionsConstants.API_GET_ATTRACTION_LIST)
                        .post(formBody)
                        .build();

                try {
                    Response response = mHttpClient.newCall(request).execute(); //setp 4
