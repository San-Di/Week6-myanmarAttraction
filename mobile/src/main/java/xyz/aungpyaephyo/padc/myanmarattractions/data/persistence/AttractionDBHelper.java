...
public class AttractionDBHelper extends SQLiteOpenHelper{
    ...
    private static final String SQL_CREATE_ATTRACTION_TABLE = "CREATE TABLE " + AttractionEntry.TABLE_NAME + " (" +
            ...
            " UNIQUE (" + AttractionEntry.COLUMN_TITLE + ") ON CONFLICT IGNORE" + // ******IGNORE or REPLACE ***********// unique constraints affects for whole table (Eg. each Entry must be unique in table by checking their title // the old data is replaced by new one since REPLACE command is used, or you also can ignore the new entry
            " );";

    private static final String SQL_CREATE_ATTRACTION_IMAGE_TABLE = "CREATE TABLE " + AttractionImageEntry.TABLE_NAME + " (" +
            AttractionImageEntry._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " + //ID is auto generated since it implements BaseColumns
